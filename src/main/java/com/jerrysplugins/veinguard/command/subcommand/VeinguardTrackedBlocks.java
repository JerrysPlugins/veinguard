...
import com.jerrysplugins.veinguard.core.page.TrackedBlockList;
...
public class VeinguardTrackedBlocks implements ISubCommand {

    private final VeinGuard plugin;

    private final TrackedBlockList trackedBlockList;

    public VeinguardTrackedBlocks(VeinGuard plugin) {
        this.plugin = plugin;
        this.trackedBlockList = new TrackedBlockList(plugin);
    }
    ...
    public String getDescription() {
        return "Add, remove or list the currently tracked blocks.";
    }
    ...
    public void execute(CommandManager commandManager, CommandSender sender, String[] args, boolean isConsole) {
        ...
        switch (subCommand) {
            ...
            case "list" -> {
                ...
                if (args.length == 3) {
                    int totalPages = trackedBlockList.getTotalPages();
                    try {
                ...
                if (isConsole) {
                    trackedBlockList.sendConsoleList(page);
                } else {
                    trackedBlockList.sendPlayerList((Player) sender, page);
                }
            }
