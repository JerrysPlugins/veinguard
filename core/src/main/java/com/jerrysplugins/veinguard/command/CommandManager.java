...
public class CommandManager implements CommandExecutor, TabCompleter {
    ...
    private void sendPluginInfo(CommandSender sender, boolean isConsole) {
        ...
        String latestVersion = plugin.getUpdateService().getLatestVersion() != null
                ? plugin.getUpdateService().getLatestVersion()
                : "Unknown";

        for (String line : lines) {
            String replaced = line
                    .replace("{description}", plugin.getPluginDescription())
                    .replace("{currentVersion}", plugin.getPluginVersion())
    ...
    public Map<String, ISubCommand> getSubCommands() { return this.subCommands; }
}